...
class Job_model extends CI_Model {
    ...
    function getSalaryTypes($active = NULL)
    {
        ...
        {
            $this->db->select('j.title, j.location, j.salary_from, j.salary_to, j.expiry_date, t.name AS test_name');
        ...
        {
            $this->db->select('j.title, j.location, j.salary_from, j.salary_to, j.expiry_date, t.name AS test_name, jt.name AS job_type, st.name AS salary_type');
            $this->db->from('jobs AS j');
            $this->db->join('tests AS t', 't.id = j.test_id', 'LEFT');
